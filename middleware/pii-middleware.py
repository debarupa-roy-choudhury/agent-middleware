...
agent = create_agent(
    ...
    middleware=[
        PIIMiddleware("email", strategy="redact"), # redact -> replace the value with a label
        PIIMiddleware("credit_card", strategy="mask"), # mask -> keep a hint
...
strict = create_agent(
    ...
    system_prompt="You are a support agent. Confirm what the customer told you in one line.",
    middleware=[
        PIIMiddleware("credit_card", strategy="block") # block -> will not reach to model
    ],
)
try:
