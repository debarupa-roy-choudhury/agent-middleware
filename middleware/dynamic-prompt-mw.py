...
@dynamic_prompt  # middleware hook # before the model --> create or change the system prompt
def support_prompt(request):
    ...
    # This hook runs before every model call in the agent loop (once before the tool call, again after the tool 
    # result comes back), so only log on the first call of the invoke to avoid printing the prompt twice.
    if not any(isinstance(msg, ToolMessage) for msg in request.messages):
